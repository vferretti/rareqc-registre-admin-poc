...
// seedAddress holds a real Quebec address with coordinates from AQRÉS.
...
// loadAddresses reads addresses from the CSV file extracted from AQRÉS.
func loadAddresses(path string) []seedAddress {
	f, err := os.Open(path)
	if err != nil {
		log.Fatalf("failed to open addresses CSV: %v", err)
	}
	defer f.Close()
	...
	for {
		...
		addrs = append(addrs, seedAddress{
			Street:     record[0],
			City:       record[1],
			PostalCode: record[2],
			Latitude:   lat,
	...
	log.Printf("Loaded %d addresses from %s", len(addrs), path)
	return addrs
}

var addresses []seedAddress

func randomAddress() seedAddress {
...
func main() {
	// Load real addresses from AQRÉS CSV
